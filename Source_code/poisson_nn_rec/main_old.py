import optparse
from ctr_old import Param, CTR_OLD
...
def run():
    ...
    nn.FLAGS.learning_rate = opts.nn_learning_rate
    ...
    param.learning_rate = opts.learning_rate
    ...
    users = Data()
    print "Loading users..."
    users.read_data(opts.user)
    num_users = len(users.m_vec_len)
    ...
    if len(opts.mult) > 1:
        ctr.read_init_information(opts.theta_init, None, opts.alpha_smooth)
